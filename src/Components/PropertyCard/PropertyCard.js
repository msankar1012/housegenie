    ...
    return (
        <div className="hg-card">
            <figure className={'mb-0'}>
                {(images.length > 0) ? <img src={`https://cdn.repliers.io/${images[0]}`} alt={mlsNumber} /> : <img src={imgPlaceholder} alt={mlsNumber} />}
            </figure>

            <div className={'hg-card-body position-relative'}>
                <div className={'price-txt mb-3'}>${listPrice}</div>
                <p className={'pb-1'}>
                    <span className={'me-3'}>{details.numBedrooms} Beds </span>
                    <span className={'me-3'}>{details.numBathrooms} Bath </span>
                </p>
                <p className={'pb-0'}>{address.streetNumber} {address.streetName}, {address.city}</p>
                <Link to={`/property/${mlsNumber}`} className={'link-property'}>
